...
// Header that hides when the user is scrolling down. 
const DynamicHeader = () => {
  const { logout } = useLogout();
  const { showHeader } = useScrollHeader();

  return (
    <header role="banner" aria-labelledby="odinbook-logo" className={`h-[50px] transition-all duration-300 shadow-md fixed top-0 w-full bg-white grid grid-cols-1 lg:grid-cols-3 content-center justify-items-center z-20 ${showHeader ? 'top-0' : 'top-[-50px]'}`}>
