...
const ProfileNav = ({ profileUser }) => {
  return (
    <div className='flex items-center justify-center mx-4 border-t mt-6 lg:justify-start lg:mx-0 w-full max-w-xl lg:max-w-4xl'>
      ...
      <Link className={`w-20 text-center border-2 border-transparent text-sm py-3 hover:bg-gray-100  ${window.location.pathname.includes('friends') ? 'text-plum-500 font-semibold bg-plum-50 border-b-plum-500' : 'font-medium text-gray-600'}`} to="friends">Friends</Link>
    </div>
  )
}
