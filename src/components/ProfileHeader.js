...
const ProfileHeader = ({ profileUser, profileType }) => {
  ...
  return (
    <section className="shadow-sm mb-6 w-full relative bg-white flex flex-col items-center justify-center">
      <div className="bg-gradient-to-t from-plum-50 h-28 md:h-36 relative w-full">
        {/* An essential div for styling purposes. Serves as cover image/background */}
      </div>
      ...
      {/* Modals */}
      {showPicModal && (<ProfilePicModal closeModal={() => setShowPicModal(false)} profileUser={profileUser} />)}
      {showEditModal && (<EditProfileModal closeModal={() => setShowEditModal(false)} profileUser={profileUser} />)}
    </section>
  )
}
