const Toast = ({ visible, params }) => {
  ...
  return (
    <div role="status" className={`flex items-center justify-start ${params.type === 'error' && 'bg-red-600'} ${params.type === 'success' && 'bg-green-600'} text-white mb-24 py-3 pl-4 w-72 rounded shadow-[0_5px_5px_-3px_rgb(0,0,0,0.16),_0_8px_10px_1px_rgb(0,0,0,0.11),_0_3px_14px_2px_rgb(0,0,0,0.10)] fixed top-20 right-4 z-100 transition-opacity ${visible ? 'opacity-100' : 'opacity-0'}`}>
      <div className="mr-4">
        {params.type === 'error' && (
