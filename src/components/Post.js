...
const Post = ({ post }) => {
  const { likePost, response: likeResponse, loading: likeLoading, error: likeError } = useLikePost();
  ...
  useEffect(() => {
    if (likeError) {
      showToast('error', likeError.errorMsg);
      setLocalLike(0);
    }
  }, [likeError, showToast]);
  ...
  return (
    <article className="rounded shadow-sm bg-white mb-6 pt-3 pb-1">
      ...
      <div className='flex items-center justify-evenly mx-4 border-t pt-1'>
        <Button customStyles="flex items-middle justify-center text-gray-500 font-medium hover:bg-gray-100 py-2 rounded-lg w-full" onClick={() => {
          likePost(post._id);
          setLocalLike(1);
        }}>
        ...
        <Button customStyles="flex items-middle justify-center text-gray-500 font-medium hover:bg-gray-100 py-2 rounded-lg w-full">
